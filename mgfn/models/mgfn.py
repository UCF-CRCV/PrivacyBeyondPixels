...
from mgfn.utils.utils import FeedForward,LayerNorm, GLANCE,FOCUS
...
def MSNSD(features,scores,bs,batch_size,drop_out,ncrops,k):
    ...
    idx_abn = torch.topk(afea_magnitudes_drop, min(k, afea_magnitudes_drop.shape[1]), dim=1)[1]
    ...
    idx_normal = torch.topk(nfea_magnitudes_drop, min(k, nfea_magnitudes_drop.shape[1]), dim=1)[1]
...
class mgfn(nn.Module):
    ...
    def forward(self, video):
        ...
        x_f = x[:,:args.feature_size,:]
        x_m = x[:,args.feature_size:,:]
