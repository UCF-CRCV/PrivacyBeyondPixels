...
def load_pretrained(
    ...
    pretrained = os.path.join(pretrained, glob.glob(os.path.join(pretrained, 'vit*.pth.tar'))[0])
...
def init_encoder(
    ...
    pretrained='/path/to/vit*.pth.tar',
    model_name='vit_huge',
    patch_size=16,
    crop_size=224,
...
def init_classifier(encoder, num_classes=400, depth=1, device=torch.device('cuda:0'), weights_path='/path/to/k400-probe.pth.tar'):
    ...
    classifier = load_pretrained(encoder=classifier, pretrained=weights_path, checkpoint_key='classifier')
    return classifier

def init_vjepa(weights_path='/path/to/vit*.pth.tar'):
    encoder = init_encoder(pretrained=weights_path)
    classifier = init_classifier(encoder, weights_path=weights_path)
