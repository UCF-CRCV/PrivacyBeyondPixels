...
class ucf_frames_dataset(Dataset):
    def __init__(self, reverse=False, dataset='ucf101_mae_train', fb_extract=False):
        self.split = 'train' if 'train' in dataset else 'test'
        if 'vp' in dataset:
            ...
            all_paths = [x.replace('/', os.sep) for x in all_paths] if self.split == 'test' else  [x.replace('/', os.sep).split(' ')[0] for x in all_paths]
            ...
            self.video_list = [os.path.join(cfg.ucf101_path, 'Videos', x.replace('/', os.sep).split(' ')[0]) for x in self.all_paths]
        else:
            # self.video_list = glob.glob(os.path.join(cfg.ucf101_path, 'Videos', '*', '*'), recursive=False)
            all_paths = open(os.path.join(cfg.ucf101_path, 'ucfTrainTestlist', f'{self.split}list01.txt'),'r').read().splitlines()
            self.video_list = [os.path.join(cfg.ucf101_path, 'Videos', x.replace('/', os.sep).split(' ')[0]) for x in all_paths]
        if reverse:
