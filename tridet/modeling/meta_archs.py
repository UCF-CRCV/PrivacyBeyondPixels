...
class TriDet(nn.Module):
    ...
    def decode_offset(self, out_offsets, pred_start_neighbours, pred_end_neighbours):
        ...
        if not self.use_trident_head:
            if self.training or len(out_offsets) > 1:
            ...
            # print(self.training, len(out_offsets))
            # TODO: this is a bit hacky, we should have a better way to handle this.
            if self.training or len(out_offsets) == 6:
                out_offsets = torch.cat(out_offsets, dim=1)
    ...
    def forward(self, video_list):
        ...
        if self.training or len(video_list) > 1:
